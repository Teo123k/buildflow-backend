import { generateFixPrompt as buildPlannerGenerateFixPrompt } from "./build_planner.js";
...
function generateBuildPrompt(step, context = "") {
  ...
  if (Array.isArray(step.files_to_edit) && step.files_to_edit.length) {
    notes.push("Files: " + step.files_to_edit.join(", "));
  }

  if (Array.isArray(step.micro_step_instructions) && step.micro_step_instructions.length) {
  ...
  return [
    {
      ...
      files_to_edit: ["main.ts"], // keep as original
      micro_step_instructions: [
        "Create entry file",
        "Add health endpoint",
        "Run server",
      ],
      replit_prompt: `Create a minimal backend for: ${String(idea || "").slice(0, 80)}`,
...
function calculatePhaseProgress(phases, steps) {
  ...
  return phases.map((p) => {
    ...
    const completed = p.steps.filter((id) => status[id] === "completed").length;
  ...
  try {
    ...
    const workflow = {
      ...
      progress: {
        ...
        current_step:
          steps.find((s) => s.status !== "completed")?.order || 1,
        next_step:
          steps.find((s) => s.status !== "completed")?.id || 0,
      },
      phase,
      progress_hint:
